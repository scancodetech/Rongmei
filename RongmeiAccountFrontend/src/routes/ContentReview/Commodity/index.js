...
class ContentReviewCommodityPage extends React.Component {
    ...
    submitValue = async () => {
        ...
        // 信息提交
        try {
            await this.draftService.reviewDraft(this.state.id, 0, this.state.isApprove, this.state.msg);
            message.success("提交成功")
            await this.noticeService.sendNotice("素材审批", this.state.isApprove ? `${this.state.title}素材审批通过` : `${this.state.title}素材审批不通过,${this.state.msg}`, "13700000002", this.state.author, 0);
...
export default ContentReviewCommodityPage
