...
class ExamExerciseCheckPage extends React.Component {
    ...
    columns = [
        {
            ...
            dataIndex: 'status',
            key: 'status',
            render: (status, data) => status === 0 ? <Tag color="blue">待审核</Tag> : <Tag color="red">已拒绝</Tag>,
        }, {
            title: '题干',
            dataIndex: 'question',
            key: 'question',
        ...
        },
        {
            ...
            render: (text, data) => {
                return <Space size="middle">
                    <a onClick={async () => {
                        this.updateStatus(data.id, true);
                    }}>同意</a>
                    <a style={{color: 'red'}} onClick={() => {
                        this.updateStatus(data.id, false);
                    }}>拒绝</a>
                </Space>
            },
        }];

    updateStatus = async (id, isApprove) => {
        try {
            await this.examService.approveThinkTankExercise(id, isApprove)
    ...
    async componentDidMount() {
        try {
            let exerciseItems = (await this.examService.getAllThinkTankExercises()).thinkTankExerciseItems;
            this.setState({
                data: exerciseItems
            })
            ...
            <div>
                ...
                >
                    ...
                    >
                        ...
                        <Form.Item label="选项">
                            <Select value={this.state.selections} mode="tags" placeholder="请输入选项（以回车分隔）"
                                    onChange={(value) => {
                                        this.setState({
                                            selections: value
                                        })
...
export default ExamExerciseCheckPage
