...
export async function getOrders(userGroupId: number) {
  return await request(API_SERVICE,`order?userGroupId=${userGroupId}`, {
    method: 'GET'
  })
}

export async function getOrder(orderId: number) {
  return await request(API_SERVICE,`order/${orderId}`, {
    method: 'GET'
  })
}
