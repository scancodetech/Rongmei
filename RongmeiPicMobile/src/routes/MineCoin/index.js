...
import {isWechatBrowser, randomWord, uuid} from "../../utils/utils";
...
class MineCoin extends React.Component {
    ...
    async getWechatQrcode() {
        if (isWechatBrowser()) {
            ...
            const signatureRes = await api.accountService.packWechatSignature(nonceStr, timestamp, window.location.href.split('#')[0]);
            let weChatState = {
                debug: true,
                appId: "wxed275441ac17bc31",
                timestamp: timestamp,
                nonceStr: nonceStr,
            ...
            const appId = "wxed275441ac17bc31"
            const redirectUri = encodeURI("https://m.dimension.pub/#/picmobile/me/coin");
                ...
                <WingBlank size='lg'>
                    ...
                    <div style={{height: '80px', paddingTop: '30px', textAlign: 'center'}}>
                        <a style={{
                           ...
                           onClick={() => {
                               this.props.history.push('/me/withdrawal')
                           }}
                        >提现</a>
                        <a style={{
                            color: 'white',
